...
int dot_opencl(unsigned int N) {
  ...
  cl_uint num_devices;
  err = clGetDeviceIDs(my_platform, CL_DEVICE_TYPE_ALL, 42, device_ids,
                       &num_devices);
  OPENCL_ERR_CHECK(err);
  ...
  err = clGetDeviceInfo(my_device_id, CL_DEVICE_NAME, sizeof(char) * 63,
  ...
  cl_context my_context =
      clCreateContext(0, 1, &my_device_id, NULL, NULL, &err);
  OPENCL_ERR_CHECK(err);
  ...
  cl_command_queue my_queue =
      clCreateCommandQueueWithProperties(my_context, my_device_id, 0, &err);
  OPENCL_ERR_CHECK(err);
  ...
  cl_program prog = clCreateProgramWithSource(my_context, 1, &my_opencl_program,
                                              &source_len, &err);
  OPENCL_ERR_CHECK(err);
  ...
  if (err != CL_SUCCESS) {
    ...
    char *build_log;
    size_t ret_val_size;
    err = clGetProgramBuildInfo(prog, my_device_id, CL_PROGRAM_BUILD_LOG, 0,
                                NULL, &ret_val_size);
    build_log = (char *)malloc(sizeof(char) * (ret_val_size + 1));
    err = clGetProgramBuildInfo(prog, my_device_id, CL_PROGRAM_BUILD_LOG,
  ...
  //
  cl_kernel my_kernel = clCreateKernel(prog, "dotProduct", &err);
  OPENCL_ERR_CHECK(err);

  std::cout << "Time to compile and create kernel: " << timer.get()
            << std::endl;
  ...
  //
  cl_mem ocl_x =
      clCreateBuffer(my_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     vector_size * sizeof(ScalarType), &(x[0]), &err);
  OPENCL_ERR_CHECK(err);
  cl_mem ocl_y =
      clCreateBuffer(my_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     vector_size * sizeof(ScalarType), &(y[0]), &err);
  OPENCL_ERR_CHECK(err);
  cl_mem ocl_partial =
      clCreateBuffer(my_context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                     256 * sizeof(ScalarType), &(y[0]), &err);
  OPENCL_ERR_CHECK(err);
  ...
  OPENCL_ERR_CHECK(err);
  err = clSetKernelArg(my_kernel, 2, sizeof(cl_mem), (void *)&ocl_partial);
  OPENCL_ERR_CHECK(err);
  ...
  //
  err = clEnqueueNDRangeKernel(my_queue, my_kernel, 1, NULL, &global_size,
  ...
  err = clEnqueueReadBuffer(my_queue, ocl_partial, CL_TRUE, 0,
                            sizeof(ScalarType) * 256, &(partial[0]), 0, NULL,
                            NULL);
  OPENCL_ERR_CHECK(err);
...
int dot_cuda(unsigned int N) {
  ...
  cudaMemcpy(&partial[0], cuda_partial, 256 * sizeof(double),
