...
//, double *init_guess)   // feel free to add a nonzero initial guess as needed
{
  ...
  hipLaunchKernelGGL(cuda_dot_product, dim3(512), dim3(512), 0, 0, N, cuda_r, cuda_r, cuda_scalar);
  ...
  while (1) {
    // line 4: A*p:
    hipLaunchKernelGGL(cuda_csr_matvec_product, dim3(512), dim3(512), 0, 0, N,
                       csr_rowoffsets, csr_colindices, csr_values, cuda_p,
                       cuda_Ap);

    // lines 5,6:
    hipMemcpy(cuda_scalar, &zero, sizeof(double), hipMemcpyHostToDevice);
    hipLaunchKernelGGL(cuda_dot_product, dim3(512), dim3(512), 0, 0, N, cuda_p,
                       cuda_Ap, cuda_scalar);
    hipMemcpy(&alpha, cuda_scalar, sizeof(double), hipMemcpyDeviceToHost);
    alpha = residual_norm_squared / alpha;

    // line 7:
    hipLaunchKernelGGL(cuda_vecadd, dim3(512), dim3(512), 0, 0, N,
                       cuda_solution, cuda_p, alpha);

    // line 8:
    hipLaunchKernelGGL(cuda_vecadd, dim3(512), dim3(512), 0, 0, N, cuda_r,
                       cuda_Ap, -alpha);

    // line 9:
    beta = residual_norm_squared;
    hipMemcpy(cuda_scalar, &zero, sizeof(double), hipMemcpyHostToDevice);
    hipLaunchKernelGGL(cuda_dot_product, dim3(512), dim3(512), 0, 0, N, cuda_r,
                       cuda_r, cuda_scalar);
    hipMemcpy(&residual_norm_squared, cuda_scalar, sizeof(double),
    ...
    // line 12:
    hipLaunchKernelGGL(cuda_vecadd2, dim3(512), dim3(512), 0, 0, N, cuda_p,
                       cuda_r, beta);
...
void solve_system(int points_per_direction) {
  ...
  hipMemcpy(cuda_csr_rowoffsets, csr_rowoffsets, sizeof(double) * (N + 1),
            hipMemcpyHostToDevice);
  hipMemcpy(cuda_csr_colindices, csr_colindices, sizeof(double) * 5 * N,
            hipMemcpyHostToDevice);
  hipMemcpy(cuda_csr_values, csr_values, sizeof(double) * 5 * N,
            hipMemcpyHostToDevice);

  //
  // Call Conjugate Gradient implementation with GPU arrays
  //
  conjugate_gradient(N, cuda_csr_rowoffsets, cuda_csr_colindices,
                     cuda_csr_values, rhs, solution);

  //
  // Check for convergence:
  //
  double residual_norm = relative_residual(N, csr_rowoffsets, csr_colindices,
