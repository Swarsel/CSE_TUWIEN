...
void solve_system(int N1, int M1) {
  ...
  int N = N1 * M1; // number of unknows to solve for
  ...
  // cudaMalloc(&cuda_csr_colindices, sizeof(double) * nval);
  ...
  // cudaMemcpy(cuda_csr_colindices, csr_colindices, sizeof(double) * nval,   cudaMemcpyHostToDevice);
...
int main(int argc, char *argv[]) {
  ...
  solve_system(N0, M0); // solves a system with 100*100 unknowns
