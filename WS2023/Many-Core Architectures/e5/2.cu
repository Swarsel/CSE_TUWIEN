...
__global__ void dot(int N, double *x, double *y, double *results) {
    ...
    if (threadIdx.x % warpSize == 0) {
        atomicAdd(&results[0], alpha1);
    }
}

__global__
void vecIterate(int N, double *out, double *in1, double *in2, double mod, bool plus)
{
    if (plus) {
...
void conjugate_gradient(size_t N,  // number of unknows
                        int *cuda_csr_rowoffsets, int *cuda_csr_colindices, double *cuda_csr_values,
                        double *rhs,
                        double *solution)
//, double *init_guess)   // feel free to add a nonzero initial guess as needed
{
    ...
    double *cuda_p, *cuda_r, *cuda_Ap, *cuda_pAp, *cuda_rr, *cuda_x, *cuda_rprp;
    ...
    while (1) {
        ...
        timer.reset();
        vecIterate<<<256, 256>>>(N, cuda_x, cuda_x, cuda_p, alpha, true);
        cudaDeviceSynchronize();
        time4 = timer.get();

        timer.reset();
        vecIterate<<<256, 256>>>(N, cuda_r, cuda_r, cuda_Ap, alpha, false);
        cudaDeviceSynchronize();
        time5 = timer.get();

        cudaMemset(cuda_rprp, 0, N);
        timer.reset();
        dot<<<256, 256>>>(N, cuda_r, cuda_r, cuda_rprp);
        cudaDeviceSynchronize();
        time6 = timer.get();
        cudaMemcpy(&rprp, cuda_rprp, sizeof(double), cudaMemcpyDeviceToHost);

        // std::cout << rprp << std::endl;

        if (rprp < 1e-6) {
            cudaMemcpy(solution, cuda_x, N * sizeof(double), cudaMemcpyDeviceToHost);
            break;
        }
        ...
        timer.reset();
        vecIterate<<<256, 256>>>(N, cuda_p, cuda_r, cuda_p, beta, true);
        cudaDeviceSynchronize();
        time7 = timer.get();
