...
int main() {
  ...
  double mean = 10.0;
  double stddev = 1.0;
  std::list<double> list(10'000);
  {
    std::mt19937 engine(1);
    std::normal_distribution<double> normal(mean, stddev);
    auto gen = [&normal, &engine]() { return normal(engine); };
    std::generate(list.begin(), list.end(), gen);
  }

  // expect construction from list w/o explicit template parameter type
  auto dist = Distribution(list);
